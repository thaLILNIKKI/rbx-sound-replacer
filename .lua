...
local function hook(sound)
    ...
    local conn
    conn = sound:GetPropertyChangedSignal("SoundId"):Connect(function()
        if not sound.Parent then conn:Disconnect() return end
        enqueue(sound)
...
local function hookall()
    task.spawn(function()
        if #parentFilterList == 0 then log("scan skipped: no parent filter set") return end
